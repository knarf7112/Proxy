    ...
    public class CardLockHandler : IHttpHandler
    {
        ...
        private TOL_Soc_Req SendAndReceiveFromAP(TOL_Soc_Req request, string serviceName)
        {
            ...
            try
            {
                ...
                log.Debug(m => m("開始送出Request : Uri({0}) data: {1}", serverUri, requestStr));
                responseBytes = Client.GetResponse(serverUri, "POST", requestBytes, out errMsg);
                if (responseBytes != null)
                {
                    responseString = Encoding.UTF8.GetString(responseBytes);
                    log.Debug(m => m("Response JsonString:{0}", responseString));
                    response = JsonConvert.DeserializeObject<TOL_Soc_Req>(responseString);
                }
                else
                {
                    log.Error(m => { m.Invoke("連線異常:" + errMsg); });
                }
