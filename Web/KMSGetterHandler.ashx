<%@ WebHandler Language="C#" Class="KMSGetterHandler" %>
...
public class KMSGetterHandler : IHttpHandler {

    private static readonly ILog log = LogManager.GetLogger(typeof(KMSGetterHandler));
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        log.Debug("[LoadKey Request] Data(length:" + inputData.Length + "):" + inputData);
        ...
        timer.Stop();
        log.Debug("[LoadKey] End Response (TimeSpend:" + (timer.ElapsedTicks / (decimal)System.Diagnostics.Stopwatch.Frequency).ToString("f3") + "s)");
    ...
    private static EskmsKeyPOCO GetResponse(EskmsKeyPOCO request)
    {
        ...
        serverConfig = Proxy.ConfigGetter.GetValue(ServiceName);
            ...
            using (SocketClient.Domain.SocketClient connectToAP = new SocketClient.Domain.SocketClient(ip, port, sendTimeout, receiveTimeout))
            {
                ...
                if (connectToAP.ConnectToServer())
                {
                    ...
                    if (responseBytes != null)
                    {
                        ...
                        log.Debug(m =>
                        {
                            m.Invoke("[LoadKey]Response JsonString:\n DiversKey:{0}",
